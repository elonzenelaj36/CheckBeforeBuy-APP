 ...
 * @returns {Promise<{imagePath: string, width: number, height: number, cached: boolean}>}
...
async function removeBackground({ imagePath }) {
  ...
  if (fs.existsSync(outputPath)) {
    ...
    return { imagePath: relativePath, width: meta.width, height: meta.height, cached: true };
  }

  const transparent = await removeWithClearBackdrop(input, path.basename(imagePath), mime);
  const layer = await toLayerPng(transparent);
  ...
  return { imagePath: relativePath, width: layer.width, height: layer.height, cached: false };
}
